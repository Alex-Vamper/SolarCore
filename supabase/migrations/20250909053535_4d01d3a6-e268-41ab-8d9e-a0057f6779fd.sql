-- Add device types for safety and security systems using existing schema
INSERT INTO admin.device_types (device_class, device_series, actions) VALUES
('safety', 'fire_detection', '{"status": ["safe", "alert", "active"], "flame_status": ["clear", "flames"], "temperature": "number", "smoke_percentage": "number"}'),
('safety', 'rain_detection', '{"status": ["safe", "alert"], "rain_detected": "boolean", "window_status": ["open", "closed"]}'),
('safety', 'gas_leak', '{"status": ["safe", "alert"], "gas_level": "number", "sensitivity": ["low", "medium", "high"]}'),
('safety', 'water_overflow', '{"status": ["safe", "alert"], "water_level": "number", "min_level": "number", "max_level": "number", "auto_pump": "boolean"}'),
('security', 'door_control', '{"lock_status": ["locked", "unlocked"], "security_mode": ["home", "away"], "last_action": "timestamp"}');
